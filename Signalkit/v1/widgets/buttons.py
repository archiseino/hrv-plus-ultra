...
<DarkRoundedButton>:
    color_normal: [0.2, 0.2, 0.2, 1]
    color_down: [0.3, 0.3, 0.3, 1]
    color_text: [1, 1, 1, 1]
...
<CircleButton>:
    ...
    canvas.before:
        Color:
            rgba: self.color_normal if self.state == 'normal' else self.color_down
        RoundedRectangle:
            pos: self.pos[0] + 12, self.pos[1] + 15
            size: 16, 16
            radius: [self.border_radius]
""")
...
class CircleButton(Button):
    """A small circular button for actions."""
...
class FlatButton(Button):
    """A flat button with transparent background."""
...
class IconButton(FlatButton):
    """A flat button that displays an icon."""
